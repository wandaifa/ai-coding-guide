...
- 让 Claude 帮你建，或手写 `.claude/agents/name.md` 配置文件，各字段是干嘛的
...
并行有两个官方护栏，防的是「失控扇出」，知道有这回事就行：**同时跑着的子代理有并发上限**（v2.1.217 起默认 20 个，超出的排队，用 `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` 可调）；**每会话累计派生总数也曾经有过 200 个的上限**（v2.1.212 引入，`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` 可调，`/clear` 会重置这笔预算）——但注意，**这个总数上限从 v2.1.224 起被官方移除了**，长会话不会再被拒，并发和深度限制仍然保留。

顺带澄清一个说法变过几回的事：**子代理能不能再派生子代理（嵌套）**。嵌套派生 v2.1.172 就支持了（当时上限 5 层、固定不可调），v2.1.217 一度改成默认禁止，v2.1.219 又恢复默认允许、并把上限降到了 **3 层**。当前官方口径是：**默认最多嵌套 3 层，且可以用 `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` 调整**——设成 `1` 就彻底关掉嵌套；深度到顶的子代理拿不到 `Agent` 工具，自然没法再往下派；想单独禁掉某个子代理的派生能力，从它的 `tools` 里去掉 `Agent` 就行。
...
讲完「该不该用」，来动手建一个。**最省事的方式是直接用大白话让 Claude 帮你建**——你说清要个什么样的助手、存到哪，它会帮你把配置文件写好，不用你手写一个字的 frontmatter。
...
Claude 会在 `~/.claude/agents/` 下建好 `code-improver.md`，名字、描述、人设提示都替你写妥当。建完打开那个文件核对一眼，不满意就让它接着改。

> ℹ️ 一个版本差异要知道：**v2.1.198 起，老的 `/agents` 交互式向导被移除了**——现在敲 `/agents` 只会提示你「让 Claude 帮你建、或直接编辑 `.claude/agents/` 目录」。v2.1.197 及更早版本里那个带 Running / Library 标签的图形化建代理界面，已经没了。

两个要点，建的时候记住：

1. **选位置**：存 `~/.claude/agents/` 是 Personal，**所有项目都能用**；存项目的 `.claude/agents/` 则只在当前项目用、还能跟着 git 提交给团队（这俩的区别下一节细说）。
2. **主动砍工具**：官方提醒了一句关键的——不主动限制，subagent 会继承主对话可用的所有工具。审查员只该读、不该写，建的时候就说清「只给只读工具」，生成的 frontmatter 里就会带上 `tools: Read, Grep, Glob` 这类限制。模型也能单独指定（比如分析类的活配 Sonnet）。

**类比：你跟行政口述一份「外包岗位需求」。** 你不用懂合同怎么写——说清楚这岗位叫啥、干啥活、能动哪些工具、用哪个档次的人，行政（Claude）帮你把单子填好、归档到正确的柜子（`~/.claude/agents/` 或项目目录），你最后签字核对就行。

**真实场景**：一个很值的常驻子代理就是这么建出来的——一个 `test-runner`，专门「跑测试、只回报挂了哪些 + 错误信息」。建的时候特意只给它 Read 和 Bash，不给 Write，**就是怕它跑着跑着手痒去改你的代码**。建一次，之后天天用。

> 💡 一句话总结：建子代理的推荐方式是**口述给 Claude 让它写配置文件**（`/agents` 老向导已于 v2.1.198 移除）；**记住主动砍工具，不砍它就继承主对话的全部权限**。

---

## 04 怎么建：手写一个配置文件

让 Claude 建好之后，它落到磁盘上其实就是**一个 Markdown 文件**。看懂这个文件，你也能直接手写、或者改别人的。
...
最后一个**新手可能踩的坑**，官方专门提醒过——如果 Claude 找不到你刚建的子代理，重启 Claude Code 再试。这通常只发生在一种情况：**会话启动时 `~/.claude/agents/`（或项目的 `.claude/agents/`）这个目录还不存在**，运行中的会话不会发现中途新建的目录。第一次建子代理碰上叫它没反应，先想想是不是这茬，退出重进就好。
...
> ⚠️ 如果敲 `@agent-code-reviewer` 时列表里没有它，多半是**子代理目录加载没生效**——会话启动时 `.claude/agents/` 目录还不存在的话，运行中的会话发现不了它（第 04 节那个坑）。退出重进就好。
...
| 怎么建 | 口述给 Claude 让它建，或手写 `.claude/agents/name.md` | 必填只有 `name`/`description`；目录是新建的可能要重启会话 |
| 怎么触发 | `description` 自动派单 / 自然语言 / `@` 点名 | `@` 保证用它；description 写清楚才派得准 |

**你现在应该能：** 判断一个任务该不该外包给子代理（而不是一激动就拆），让 Claude 帮你或手写文件建一个带专属人设、限定工具的子代理，并用自动委派或 `@` 点名让它干活、把结论干净地交回主线。**这套「该外包就外包、该自己干就自己干」的分寸感，才是子代理真正的门槛——功能十分钟学会，分寸得用出来。**

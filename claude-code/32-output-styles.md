...
- Claude Code 自带的几种内置样式（默认、Proactive、Explanatory、Learning、Concise）各是干嘛的、什么时候切
...
除了默认，官方还内置了**四档**额外样式，我挨个用一句话讲清它「跟默认比，多了点啥」：
...
**Concise（简洁型）——先说结果，砍掉铺垫和旁白。** v2.1.237 起新增的内置样式：Claude 上来直接给结果，跳过开场白和过程叙述，但活儿本身干得一样扎实。适合「别解释，直接给答案」的场景。

把这几档落到一句真实的话里，你会更有体感：
...
- 同一个需求，**Concise** 下它只回一句「已加分页，用的游标方案」，没有铺垫。
...
| **Concise** | 先说结果、砍掉铺垫和叙述（v2.1.237 起） | 你只要答案、嫌它话多 | **更短** |
...
> 💡 一句话总结：内置五档——**Default 埋头干活、Proactive 放开手脚少确认、Explanatory 边干边讲、Learning 边讲边留作业、Concise 只给结果（v2.1.237 起）**；Explanatory 和 Learning 响应更长更费 token，按需切、用完切回默认。
...
值就填样式的名字（内置的填 `Explanatory`、`Learning`、`Proactive`、`Concise`，或自定义样式的名字）。这个字段写在哪个 settings 文件里，就决定了它的作用范围——想全局默认就写进用户级 `~/.claude/settings.json`，想这个项目专属就写进项目级。**谁压谁的优先级规则，跟第 31 篇讲的完全一致**，这里不重复。
...
内置五档不够用？你完全可以**自己写一个**。好消息是，门槛低得出奇——**一个 Markdown 文件就是一个输出样式**。
...
**你现在应该能：** 一句话说清 output styles 改的是什么、内置五档分别什么时候切、用 `/config`（而不是已废弃的 `/output-style`）把样式切对并知道要 `/clear` 才生效；自己写一个 Markdown 自定义样式、把 `keep-coding-instructions` 这个开关按对，还能拎清它跟 CLAUDE.md、Skill、Subagent 的边界。**说白了，你手里这下多了一把「给 Claude 换人格」的钥匙——同一个模型，能让它从工程师切成导师、切成写作助手，按活儿换装。**

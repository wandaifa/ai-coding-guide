...
v2.1.187 起还有个更专门的入口：**`sandbox.credentials` 设置**——单独声明要保护的凭证文件和私密环境变量，比通用 `denyRead` 规整。文件条目设成 `"mode": "deny"` 就按 `denyRead` 同样的力度拒读；环境变量条目还能设 `"mode": "mask"`（掩码）——沙箱命令拿到的是掩码后的假值，只有发往你指定主机的请求才会被代理换上真凭证（v2.1.224 起还补了 JWT 感知掩码、AWS SigV4 重签等高级选项）。注意一条安全设计：**掩码类配置只在用户级、managed 或 `--settings` 传入的设置里生效，写在项目仓库的 `.claude/settings.json` 里会被忽略**——防止来路不明的仓库给自己开口子。
...
两个进阶开关也记一下：**`sandbox.filesystem.disabled`**（v2.1.216 起）可以**只关掉文件系统隔离、保留网络出口管控**——某些工具链跟文件系统沙箱犯冲时用得上；**`sandbox.network.strictAllowlist`**（v2.1.219 起）则是反过来把网络收到最紧——**不在允许清单里的域名直接拒、连问都不问**，适合无人值守场景。

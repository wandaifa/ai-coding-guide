...
有段时间我把默认模型锁死旗舰款、推理强度直接拉满到 `xhigh`，图个「一步到位、再不折腾」。直到有天我让它帮我把一个变量名从 `data` 改成 `payload`——一个三十秒的体力活。它「深思熟虑」了将近一分钟才肯动手，我盯着那个转圈的推理动画，第一次觉得「最强」有点蠢。
...
- Codex 当前能选的几个模型分别是什么定位、哪个是默认、哪些名字已经被淘汰
...
当前 Codex 的几个模型，定位大致是这样（GPT-5.6 一代，2026 年 7 月起全量上线）：

| 模型 | 定位 | 最适合 | 速度 / 成本 |
| --- | --- | --- | --- |
| **`gpt-5.6-sol`** | 旗舰、默认推荐 | 复杂编程、计算机操作、研究、网络安全等硬活 | 较慢 / 最高 |
| **`gpt-5.6-terra`** | 日常主力 | 日常开发，性能与上一代旗舰 GPT-5.5 相当、成本更低 | 中等 / 中 |
| **`gpt-5.6-luna`** | 轻量、快而省 | 明确、可重复的批量任务，给子代理用 | 快 / 最低 |
| **`gpt-5.3-codex-spark`** | 即时型（研究预览） | 近乎实时的高频编程迭代（偏纯文本、能力较弱） | 极快 / 低 |

几个要点，记下来少踩坑：

- **不指定就走默认。** 你要是从没设过，Codex（App / CLI / IDE 扩展）会自动用推荐的旗舰模型——配置里写 `model = "gpt-5.6"` 时指向的就是 Sol，多数情况这就够用。
- **拿不准从哪个入手，官方的建议是先用 Sol 找手感**；日常高频的活觉得它「用力过猛」，再降 Terra；任务明确又量大，直接 Luna。
- **`gpt-5.3-codex-spark` 是研究预览（research preview），目前只对 ChatGPT Pro 订阅开放。** 没看到它很正常，不是你装错了。
- **有几个名字你可能在老教程里见过，现在别用了**：`gpt-5.2` 和 `gpt-5.3-codex` 早已被官方标为弃用（deprecated）；**`gpt-5.4` 和 `gpt-5.4-mini` 也已于 2026 年 8 月 31 日在 ChatGPT 登录的 Codex 里退役**——官方指定的接班分别是 `gpt-5.6-terra` 和 `gpt-5.6-luna`。如果你的脚本、`config.toml` 或 `codex exec --model` 里还写着这些旧名字，趁早换掉。用 API key 登录的不受这次退役影响，可用模型以官方 API 模型页为准。

我自己最常用的一个组合是：**主力会话挂旗舰，批量脏活交给子代理 + `gpt-5.6-luna`。** 前阵子清理一个项目里二十多个文件的过期 import，我让主代理拆活、十几个子代理并行用 Luna 去改，几分钟全收工——这种「又多又简单」的活儿，Luna 的速度和成本优势就出来了，换旗舰纯属烧钱等结果。

> 💡 一句话总结：记定位别记参数——Sol 啃硬活、Terra 日常主力、Luna 快省跑批量、spark 求秒回，退役的旧名字别再写进配置。

![模型档位怎么挑](assets/30-model-tiers@2x.png)

把四个模型摆到「任务难度 × 速度成本」这张图上一看就清楚：越往右下角任务越硬、模型越强但越慢越贵（旗舰 `gpt-5.6-sol`），越往左上角越快越省（Luna、spark）——挑模型就是拿手头这活儿的位置去对号入座。
...
GPT-5.6 这一代还在强度表之外加了两个「超限档」，知道有就行：**Max** 是给单个任务更长的推理时间，适合「深度比速度和额度重要」的最硬问题（看不到它就去设置里开）；**Ultra** 更激进——直接派多个子代理把复杂任务拆开并行跑，适合能明显拆成几部分的大活，同样需要在设置里开启才会出现在模型滑块上。大多数任务用不到这两档，别默认开。
...
| 场景 | 推荐模型 | 推理强度 |
| --- | --- | --- |
| 架构设计、技术选型 | `gpt-5.6-sol` | `high` / `xhigh` |
| 跨模块重构、复杂改造 | `gpt-5.6-sol` | `high` |
| 难缠 / 偶发 bug 调试 | `gpt-5.6-sol` | `high` |
| 日常写功能、补逻辑 | `gpt-5.6-terra` | `medium` |
| 改小 bug、调格式、补注释 | `gpt-5.6-luna` | `low` |
| 子代理批量处理简单任务 | `gpt-5.6-luna` | `low` / `medium` |
| 实时结对、高频快问快答 | `gpt-5.3-codex-spark` | `minimal` |

怎么用这张表？**别一开始就纠结，先按默认档（`gpt-5.6`，即 Sol + `medium`）干着，碰到具体卡点再针对性调**：

- 它给的方案太浅、漏了边界情况 → 把强度提到 `high`。
- 日常高频使用觉得旗舰「用力过猛」、等得心焦 → 换 `gpt-5.6-terra`，性能够用还更省。
- 一堆又多又简单的体力活拖慢了你 → 换 `gpt-5.6-luna` 降档批量跑。
- 你只是想连珠炮式问几个小问题、等不及它思考 → 上即时型模型、`minimal`。

我的真实习惯是：**默认配置（Sol + `medium`）打天下，绝大多数活儿不用动；只有进入「日常嫌它太重」「批量跑脏活」这两种模式时，我才会手动切一下。** 把切换当成工具箱里按需取用的螺丝刀，不是时刻握在手里的锤子——想起来才拿，不用每次都纠结。

> 💡 一句话总结：默认 Sol + medium 打天下，日常嫌重用 Terra、批脏活用 Luna，只在需要时手动切档。
...
**④ 给子代理单独配（分工干活）。** 主代理用旗舰统筹，把简单的批量活儿派给挂着 `gpt-5.6-luna` 的子代理——这是性价比最高的打法，细节见〔[21 子代理（Subagents）](21-subagents.md) 〕。
...
> 💡 一句话总结：一劳永逸改 config、临时起活用 `-m`、中途换人敲 `/model`、杂活外包给 Luna 子代理。
...
- **认识当前模型**：旗舰 `gpt-5.6-sol` 啃硬活、`gpt-5.6-terra` 日常主力、`gpt-5.6-luna` 快省跑批量、`gpt-5.3-codex-spark` 求秒回；`gpt-5.2`、`gpt-5.3-codex` 已弃用，`gpt-5.4`、`gpt-5.4-mini` 也已退役，别再写进配置。
- **推理强度是隐藏王牌**：嫌笨往上提一档、嫌慢往下降一档，常比换模型更快见效；最硬的活还有 Max / Ultra 两个超限档兜底。
- **一张对照表 + 一个习惯**：默认 Sol + `medium` 打天下，日常嫌重切 Terra、批脏活切 Luna。

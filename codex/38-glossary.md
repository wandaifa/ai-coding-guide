...
- 模型体系（`gpt-5.6` 这一串）的定位区分、「研究预览」什么意思、哪些旧名字已弃用 / 退役别再用
...
一句话：**当前推荐的旗舰模型，最强也最贵，不设置就默认用它（配置里写 `gpt-5.6` 指向的就是它）。**

在 Codex 里指什么：复杂编程、跨模块重构、难缠 bug、研究类工作流，派它准没错。多数情况这就够用，你从没设过就是它。

**`gpt-5.6-terra`（日常主力）**

一句话：**日常开发的性价比之选，性能与上一代旗舰 GPT-5.5 相当、成本更低。**

在 Codex 里指什么：日常写功能、补逻辑这类「每天都要干」的活，觉得旗舰用力过猛就换它。它也是已退役的 `gpt-5.4` 的官方接班。

**`gpt-5.6-luna`（轻量、快而省）**

一句话：**轻量模型，跑得快、花得少，适合明确、可重复的批量活儿，也常给子代理用。**

在 Codex 里指什么：改小 bug、调格式、补注释、批量清理这类「体力活」，派它比派旗舰省钱省时间。它是已退役的 `gpt-5.4-mini` 的官方接班。
...
**已弃用 / 已退役模型（Deprecated / Retired）**

一句话：**官方已经标为淘汰、不再推荐的旧模型，别再写进配置。**

在 Codex 里指什么：`gpt-5.2` 和 `gpt-5.3-codex` 在 ChatGPT 登录方式下早已被官方标为弃用；**`gpt-5.4` 和 `gpt-5.4-mini` 也于 2026 年 8 月 31 日在 ChatGPT 登录的 Codex 里退役**，官方指定的接班分别是 `gpt-5.6-terra` 和 `gpt-5.6-luna`；上一代旗舰 `gpt-5.5` 目前还能用，但新活儿直接上 5.6 一代。**如果你的脚本、`config.toml` 或 `codex exec --model` 里还写着这些旧名字，趁早换掉。** 我自己上个月就栽过——一个半年前写的自动化脚本里硬编码了老模型名，跑起来直接报模型不可用，翻了半天才想起是这茬。

| 模型 | 定位 | 最适合 |
| --- | --- | --- |
| `gpt-5.6-sol` | 旗舰、默认 | 复杂编程、重构、难缠 bug、研究 |
| `gpt-5.6-terra` | 日常主力 | 日常开发，平衡质量与成本 |
| `gpt-5.6-luna` | 轻量、快省 | 简单批量活、给子代理用 |
| `gpt-5.3-codex-spark` | 即时型（研究预览） | 高频实时迭代，求秒回 |
| `gpt-5.5` | 上一代旗舰 | 仍可用，新活儿建议直接上 5.6 |
| `gpt-5.2` / `gpt-5.3-codex` / `gpt-5.4` / `gpt-5.4-mini` | 已弃用 / 已退役 | 别再用，换成 5.6 一代 |

> 💡 一句话总结：`gpt-5.6-sol` 啃硬活、`gpt-5.6-terra` 日常主力、`gpt-5.6-luna` 跑快省、`gpt-5.3-codex-spark` 求秒回；「研究预览」=尝鲜版，弃用和退役的旧名字别再写进配置。
...
- **模型相关**：`gpt-5.6` 这一串的定位、研究预览是啥、哪些旧名字已弃用 / 退役。

...
模型选什么、推理强度配多大，这条链路里我用的就是默认那套——**旗舰 `gpt-5.6-sol` + 默认推理强度（一般是 `medium`）**，这个 TODO 小活儿完全够用，没必要拉到 `high`。要是你碰上的是「跨模块大重构」那种硬骨头，再回〔[30 怎么选模型](30-models.md) 〕按场景调档。**这一篇我刻意全程用默认配置走，就是想让你看清：链路本身才是主角，调参是锦上添花。**

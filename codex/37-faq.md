...
- **以 `/model` 面板实际列出的为准，别背名字。** 当前体系（GPT-5.6 一代）里旗舰默认是 `gpt-5.6-sol`，日常主力 `gpt-5.6-terra`，轻量、给子代理用的是 `gpt-5.6-luna`。这几个绝大多数账号都有。
- **看不到 `gpt-5.3-codex-spark` 很正常**——它是即时型研究预览，目前只对 ChatGPT Pro 订阅开放，没有不代表你装错了。
- **配置里写了某个模型却报不可用**，先翻你的 `~/.codex/config.toml` 和 `codex exec --model` 参数，看是不是还写着已弃用 / 已退役的老名字——`gpt-5.2`、`gpt-5.3-codex` 早已被弃用，`gpt-5.4`、`gpt-5.4-mini` 也已于 2026 年 8 月 31 日在 ChatGPT 登录下退役。换成 5.6 一代（`gpt-5.6` / `gpt-5.6-terra` / `gpt-5.6-luna`）即可。
...
- **订阅被限流**，要么等额度刷新，要么升套餐。日常省额度的关键是别无脑顶格——简单活儿用 `gpt-5.6-luna`、低推理强度，把旗舰 + 高推理留给真正的硬骨头。这套在〔[30 怎么选模型](30-models.md) 〕讲透了。
...
- **批量任务交给子代理 + 轻量模型。** 又多又简单的活儿（比如批量改文件名、清过期 import），Luna 的速度和成本优势就出来了。

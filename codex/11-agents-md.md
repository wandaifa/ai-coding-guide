...
还有一个安全相关的边界要知道（0.150.0 起收紧）：**被标为「不可信（untrusted）」的项目，Codex 不再注入它的项目级 `AGENTS.md` 指令**——全局层照常读，项目层整段跳过。道理和项目级配置的信任门槛一致：陌生仓库里的指令文件本质上是「别人写给你 AI 的话」，没经你信任就进上下文，等于给提示注入开了后门。所以克隆一个陌生仓库直接跑 Codex，它的 `AGENTS.md` 不会生效；确认过项目没问题、标为信任之后才会恢复注入。
